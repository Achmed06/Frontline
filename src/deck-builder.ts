...
export function renderDeckBuilder(
  ...
  container.innerHTML = `
    ...
    <section id="deck-analysis" class="deck-analysis" aria-label="Deckzusammensetzung"></section><div class="roster">${[...units, ...tactics]
      .map(
        (
          card,
        ) => `${card.id === tactics[0].id ? '<h3 class="tactics-heading">Zwei Fähigkeiten wählen</h3>' : ""}<button class="roster-card" data-mastery="${masteryRank(mastery.units[card.id] ?? 0).frame}" data-unit="${card.id}" aria-pressed="false" aria-label="${card.name} auswählen">
  ...
  const buttons = Array.from(
    container.querySelectorAll<HTMLButtonElement>("[data-unit]"),
  );
  const completeDeck = () => [
  ...
  function update(message = "") {
    buttons.forEach((button) => {
      ...
      button.setAttribute(
        "aria-label",
        `${CARDS.find((card) => card.id === button.dataset.unit)!.name} ${chosen ? "entfernen" : "auswählen"}`,
      );
    ...
    (button) =>
      (button.onclick = () => {
        ...
        const count = draft.filter(
          (selected) =>
            CARDS.find((card) => card.id === selected)!.kind === kind,
        ).length;
        if (draft.includes(id))
          ...
          update(
            "Entferne zuerst eine Karte desselben Typs, um sie auszutauschen.",
          );
          return;
        }
        update();
  ...
  container.querySelectorAll<HTMLButtonElement>("[data-preset]").forEach(
    (button) =>
      (button.onclick = () => {
  ...
  function refreshSlots(selected: number) {
    ...
    slots.forEach((slot, index) => {
      ...
      // Player-entered names are rendered as text, never HTML.
    ...
    get("deck-slot-save").textContent = slots[selected]
      ? "PLATZ ÜBERSCHREIBEN"
      : "HIER SPEICHERN";
  }
  ...
  get("deck-slot-load").onclick = () => {
    ...
    draft = [...slot.cards];
    update();
    get("deck-slot-status").textContent =
      `${slot.name} geladen. Du kannst es bearbeiten oder unten als aktives Deck übernehmen.`;
  };
