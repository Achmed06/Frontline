...
import type { BasePlotPresentation } from "./base-placement-visual";
...
export function baseMapSvg(progress: LearningProgress, accent: string, options: {
  ...
  interactive?: boolean;
  stage?: number;
  placementStates?: readonly BasePlotPresentation[];
} = {}): string {
  const {
    selected,
    placing = false,
  ...
  const plots = Array.from({ length: BASE_PLOT_COUNT }, (_, plot) => plot).sort((a, b) => plotPosition(a).y - plotPosition(b).y || a - b).map(plot => {
    ...
    const placementState = placementStates?.[plot];
    const placementClass = placementState ? ` placement-${placementState}` : "";
    const placementLabel =
      placementState === "valid" || placementState === "chosen-valid"
        ? ", gültiger Bauplatz"
        : placementState === "required"
          ? ", Ausbauposition"
          : placementState === "source"
            ? ", aktueller Standort"
            : placementState === "occupied"
              ? ", belegt"
              : placementState === "blocked" || placementState === "chosen-invalid"
    ...
    const art =
      plot === COMMAND_PLOT
        ...
        : id
          ...
          : plot === chosen && ghost
            ? `<g class="placement-ghost" opacity=".6">${buildingArt(ghost, accent)}</g>`
            : "";
    const telegraph =
      placementState === "valid" || placementState === "chosen-valid"
        ? `<path class="plot-valid-ring" d="M-38 0 0-19 38 0 0 19Z" fill="none"/><path class="plot-corners" d="M-39-3v7l7 4M39-3v7l-7 4M-14-17 0-23 14-17" fill="none"/>`
        : placementState === "required"
          ? `<path class="plot-required-ring" d="M-40 0 0-20 40 0 0 20Z" fill="none"/><circle class="plot-required-node" cy="-29" r="4"/>`
          : placementState === "source"
            ? `<path class="plot-source-ring" d="M-39 0 0-19 39 0 0 19Z" fill="none"/><path class="plot-source-mark" d="M-10 0h20M0-6v12" fill="none"/>`
            : placementState === "occupied" || placementState === "blocked" || placementState === "chosen-invalid"
              ? `<path class="plot-block-mark" d="M-7-4 7 4M7-4-7 4" fill="none"/>`
              : "";
    return `<g class="base-plot ${root ? "occupied" : "empty"} ${selectedPlot ? "selected" : ""}${placementClass}" data-plot="${plot}" transform="translate(${x} ${y})" ${interactive ? `role="button" tabindex="0" aria-label="${name}${root ? (id === root ? ', Stufe 1' : ', Stufe 2') : ''}${placementLabel}" aria-pressed="${selectedPlot}"` : ''}><path class="plot-ground" d="M-46 0 0-23 46 0 0 23Z" fill="${root || plot === COMMAND_PLOT ? '#647967' : (plot % 2 ? '#588069' : '#5e876c')}" stroke="${selectedPlot ? accent : '#9bb08a'}" stroke-opacity="${selectedPlot ? 1 : .22}" stroke-width="${selectedPlot ? 3 : 1}"/>${!root && plot !== COMMAND_PLOT ? `<path class="plot-plus" d="M-6 0h12M0-4v8" stroke="${placing ? accent : '#bdd5aa'}" stroke-width="2" opacity="${placing ? .9 : .35}"/>` : ""}${telegraph}${art}${root ? `<g transform="translate(0 21)"><rect x="-13" y="-5" width="26" height="12" rx="4" fill="#102e32" stroke="${accent}" stroke-width=".6"/><text text-anchor="middle" y="4" fill="#eaf2d6" font-size="8" font-family="sans-serif">${id === root ? 'I' : 'II'}</text></g>` : ""}</g>`;
  }).join("");
