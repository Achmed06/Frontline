...
export function saveStats(stats: LocalStats) {
  try { localStorage.setItem("frontline-stats-v1", JSON.stringify(stats)); } catch {}
}
...
export function createMatchRecord(match: Match, missionId?: string): MatchRecord {
  const state = match.state;
  if (state.phase !== "ended" || !state.winner) throw new Error("Cannot record an unfinished match");
  return {
    ...
    core: {
      player: Math.ceil(Math.max(0, state.cores.player.hp / state.cores.player.maxHp) * 100),
      enemy: Math.ceil(Math.max(0, state.cores.enemy.hp / state.cores.enemy.maxHp) * 100),
    },
    points: {
      player: state.points.filter((point) => point.owner === "player").length,
      enemy: state.points.filter((point) => point.owner === "enemy").length,
    },
    ...(match.controlObjective ? { control: { player: Math.floor(state.controlTime.player + 1e-8), enemy: Math.floor(state.controlTime.enemy + 1e-8), target: match.controlObjective.seconds } } : {}),
    feedback: "",
    ...(missionId && MISSIONS.some((mission) => mission.id === missionId) ? { missionId } : {}),
  };
}
function isMatchRecord(value: unknown): value is MatchRecord {
  ...
  return (
    ...
    (record.control === undefined ||
      (!!record.control && Number.isFinite(record.control.target) && record.control.target > 0 &&
        record.control.target <= MATCH_DURATION && integer(record.control.player, record.control.target) &&
        integer(record.control.enemy, record.control.target))) &&
    (record.missionId === undefined || MISSIONS.some((mission) => mission.id === record.missionId)) &&
    typeof record.id === "string" && /^[a-zA-Z0-9-]{1,80}$/.test(record.id) &&
    ...
    isValidDeck(record.deck) && !!record.core &&
    integer(record.core.player, 100) && integer(record.core.enemy, 100) &&
    !!record.points && integer(record.points.player, 9) && integer(record.points.enemy, 9) &&
...
export function saveHistory(history: readonly MatchRecord[]): boolean {
  ...
  if (!recent.every(isMatchRecord) || new Set(recent.map((record) => record.id)).size !== recent.length) return false;
  return setting("history-v1", JSON.stringify(recent)) !== null;
}
export function readCampaign(): CampaignProgress {
  try { return normalizeProgress(JSON.parse(setting("campaign-v1") ?? "{}")); } catch { return {}; }
}
export function saveCampaign(progress: CampaignProgress): boolean {
  return setting("campaign-v1", JSON.stringify(normalizeProgress(progress))) !== null;
}
export function readSeries(): SeriesRun | null {
  try { return normalizeSeries(JSON.parse(setting("series-v1") ?? "null")); } catch { return null; }
}
...
export function readLearning(): LearningProgress {
  try { return normalizeLearning(JSON.parse(setting("learning-v1") ?? "null")); } catch { return normalizeLearning(null); }
}
export function saveLearning(progress: LearningProgress): boolean {
  return setting("learning-v1", JSON.stringify(normalizeLearning(progress))) !== null;
}
export function readMastery(): Mastery {
  try { return normalizeMastery(JSON.parse(setting("mastery-v1") ?? "null")); } catch { return normalizeMastery(null); }
}
export function saveMastery(progress: Mastery): boolean {
  return setting("mastery-v1", JSON.stringify(normalizeMastery(progress))) !== null;
}
export function readDeckSlots(): DeckSlot[] {
  const saved = setting("deck-slots-v1");
  if (saved === null) return normalizeDeckSlots([{ name: "Mein erstes Deck", cards: readDeck() }]);
  try { return normalizeDeckSlots(JSON.parse(saved)); } catch { return normalizeDeckSlots(null); }
}
export function saveDeckSlots(slots: readonly DeckSlot[]): boolean {
  return setting("deck-slots-v1", JSON.stringify(normalizeDeckSlots(slots))) !== null;
}
