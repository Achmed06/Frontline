...
export function renderStore(container: HTMLElement, stage: number, close: () => void, equip: () => void): void {
  container.innerHTML = `<div id="store-eyebrow" class="eyebrow">UNTERSTÜTZERPAKET · APPLE STORE</div><h2>Kommandogold</h2><div class="hq-art store-preview">${headquartersSvg(stage, '#ffe17d')}</div><p>Goldene Akzente für dein Hauptquartier und deine Einsatzbasis. Einmal freischalten, dauerhaft gestalten. Keine stärkeren Truppen, keine zusätzlichen Kampfwerte.</p><p id="store-mode-note" class="store-test-note">Einmalige kosmetische Freischaltung. Keine stärkeren Truppen und keine zusätzlichen Kampfwerte.</p><p id="store-status" role="status" aria-live="polite"></p><button id="store-buy" class="primary" disabled>APPLE-SHOP LADEN …</button><button id="store-equip" class="primary" hidden>KOMMANDOGOLD AUSRÜSTEN</button><button id="store-restore" class="secondary">KÄUFE WIEDERHERSTELLEN</button><button id="store-retry" class="secondary" hidden>APPLE-SHOP ERNEUT LADEN</button><button id="store-close" class="secondary">ZURÜCK ZUM HAUPTQUARTIER</button>`;
  const eyebrow = container.querySelector<HTMLElement>('#store-eyebrow')!;
  const modeNote = container.querySelector<HTMLElement>('#store-mode-note')!;
  ...
  function update() {
    ...
    buy.textContent = storePurchaseLabel(catalog);
  }
  storeChanged = () => {
    if (!current()) return;
    interactionRevision++;
  ...
  async function load() {
    ...
    try {
      ...
      const copy = storeModeCopy(catalog);
      status.textContent = supporterOwned ? 'Kommandogold ist freigeschaltet.' : catalog.available ? copy.ready : `${copy.ready} Solo-Spielen funktioniert weiterhin.`;
