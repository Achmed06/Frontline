...
import { DUEL_SESSION_KEY, parseDuelSession, serializeDuelSession, type DuelSession } from "./duel-session";
import "./duel.css";
...
function restoreSession(): DuelSession | null {
  for (const storage of [localStorage, sessionStorage]) {
    try {
      const restored = parseDuelSession(storage.getItem(DUEL_SESSION_KEY));
      if (!restored) continue;
      // Migrate old sessionStorage-only sessions and refresh their bounded lifetime.
      try {
        localStorage.setItem(DUEL_SESSION_KEY, serializeDuelSession(restored));
      } catch {}
      return restored;
    } catch {}
  }
...
let session: DuelSession | null = restoreSession();
...
function persist() {
  const value = session ? serializeDuelSession(session) : null;
  for (const storage of [localStorage, sessionStorage]) {
    try {
      if (value) storage.setItem(DUEL_SESSION_KEY, value);
      else storage.removeItem(DUEL_SESSION_KEY);
    } catch {}
  }
}
